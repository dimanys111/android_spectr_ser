package com.example.dima.ser.audio;
...
public class AudioEncoder {
    ...
    public AudioEncoder(Context c,File f) {
        this.c = c;
        prepare(f);
    }
    ...
    private void prepare(File f) {
        ...
        audioFormat.setInteger(MediaFormat.KEY_BIT_RATE, 40000);
    ...
    public void stop(File f) {
        if (!encodingService.isShutdown())
            encodingService.submit(new EncoderTask(this, EncoderTaskType.FINALIZE_ENCODER,f));
    }
     ...
     */
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    private void _offerAudioEncoder(byte[] input, long presentationTimeNs,File f) {
    ...
    private class EncoderTask implements Runnable {
        ...
        public EncoderTask(AudioEncoder encoder, EncoderTaskType type,File f) {
            this.f=f;
            setEncoder(encoder);
            this.type = type;
            switch (type) {
        ...
        private void encodeFrame(File f) {
            if (encoder != null && audio_data != null) {
                encoder._offerAudioEncoder(audio_data, presentationTimeNs,f);
                audio_data = null;
            }
        }
